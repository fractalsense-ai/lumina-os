...
- problem_status tracking on current_task
- Fluency-gated problem advancement (tier lookup + generate_problem)
- task_presented_at timestamp reset
...
def education_post_turn(
    *,
    ...
    Returns a dict with:
        ...
        current_task:  dict — possibly replaced with a new generated problem
        new_task_presented: bool — True when a new problem was generated
    """
    ...
    if isinstance(reported_status, str) and reported_status.strip():
        current_task["status"] = reported_status.strip()

    # ── Fluency-gated problem advancement ─────────────────────
    new_task_presented = False
    fluency_decision = {}
        ...
        if isinstance(tiers, list) and tiers:
            try:
                ...
                if gen_fn is not None:
                    ...
                    current_task = gen_fn(diff, subsystem_configs, domain_id=module_key)
                    current_task["solved"] = False
    ...
    return {
        ...
        "new_task_presented": new_task_presented,
    }
...
def education_reset_timer(
    *,
    ...
    """Reset task_presented_at after response is built.
    ...
    """
    if new_task_presented or resolved_action == "task_presentation":
        _response_sent_at = time.time()
        session["task_presented_at"] = _response_sent_at
        _c = session_containers.get(session_id)
        if _c is not None:
            _c.active_context.task_presented_at = _response_sent_at
