...
- **Client-side analyzer** (`model-packs/education/web/services/vocabularyAnalyzer.ts`) — buffers student messages, computes lexical diversity, average word length, embedding spread, and domain-term detection. Posts structured metrics via the domain-declared API route.
