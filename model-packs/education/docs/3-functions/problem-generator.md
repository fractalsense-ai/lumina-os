...
`model-packs/education/reference-implementations/problem_generator.py`

## SEE ALSO

- `model-packs/education/modules/algebra-level-1/domain-physics.json` — defines `equation_difficulty_tiers` (tier boundaries and generator names)
- `model-packs/education/reference-implementations/fluency_monitor.py` — `advance_tier` action triggers a new problem generation call
- `model-packs/education/reference-implementations/runtime-adapters.py` — `domain_step()` merges fluency decision and triggers problem generation
