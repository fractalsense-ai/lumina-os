"""Math-specific NLP pre-interpreter for learning modules.

Runs answer-matching, off-task ratio (math vocabulary), frustration detection,
and hint-request detection.  Designed for algebra / pre-algebra modules where
math vocabulary overlap is a meaningful signal.
"""
...
_VARIABLE_CHAR_RE = re.compile(r"^[a-zA-Z]$")
...
def _is_math_token(token: str) -> bool:
    ...
    if _OPERATOR_RE.match(lower):
        return True
    if _VARIABLE_CHAR_RE.match(lower):
        return True
...
def math_nlp_preprocess(input_text: str, task_context: dict[str, Any]) -> dict[str, Any]:
    """Run math-specific NLP extractors and return a partial evidence dict.
