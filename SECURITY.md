...
Project Lumina is designed with **Defensive AI Architecture** at its core. Our security model assumes that Large Language Models are inherently untrustworthy and will attempt to drift. Security in Lumina is not about making the AI "safe"; it is about ensuring the **D.S.A. Orchestrator** strictly bounds the AI, and the **Causal Trace Ledger (CTL)** cryptographically records every deviation.

If a vulnerability allows the AI to bypass its prompt contract, evade the CTL, or escalate its authority without authorization, it is treated as a critical severity issue.
...
1. **AI Hallucinations / Drift:** LLMs hallucinating is expected. It is only a security vulnerability if the Orchestrator fails to halt the session, trigger an `EscalationRecord`, and log the hash to the CTL.
...
#### 2. Causal Trace Ledger (CTL) Compromise
...
**Do not open a public issue for a critical framework bypass.** If you discover a vulnerability that compromises the CTL or allows unlogged contract escapes, please report it via email to the core maintainers.
