...
| **Three-track token isolation** | System, domain, and user tokens are not interchangeable | Air-gapped three-secret JWT signing: `LUMINA_ADMIN_JWT_SECRET` (system track: root, super_admin), `LUMINA_DOMAIN_JWT_SECRET` (domain track: admin), `LUMINA_USER_JWT_SECRET` (user track: user, operator, half_operator, guest); `iss` claim distinguishes provenance; `verify_scoped_jwt()` enforces `required_scope`. See [Parallel Authority Tracks](parallel-authority-tracks.md) and [Air-Gapped Admin Architecture](../8-admin/air-gapped-admin-architecture.md) | **401/403** — cross-track tokens rejected |
...
|----------|------|----------------|
| **A01 — Broken Access Control** | Unauthorized users access sessions, System Log records, or domain physics | RBAC with chmod-style octal permissions enforced per module; `check_permission()` on every API call; `require_role()` decorators on admin endpoints; half_operator role has read-only System Log scope by design |
